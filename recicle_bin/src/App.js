...
function App() {
 ...
 {materials.find(mat=>(selects=== mat.name))}
  ...
  return (
  <main>
      <body class = "rgba(32, 126, 20, 0.493) space-y-5" >
    ...
    <h1 id = "type" class='font-normal  justify-self-center text-3xl '>    {materials.map((mat,index)=> {
      if (mat.name === selects) {
        return <div id ="mat">
        <h1>{mat.type}</h1>
        </div>
    ...
    <MapContainer class='leaflet-container'  center={[-9.701426, -36.688151]} zoom={45} scrollWheelZoom={false}>
      <TileLayer
